...
def speak_text(text, speed=1.0):
    ...
    # Wait for the speaking to be finished
    engine.runAndWait()


speak_text("Hmm, sure, Let me take you to the manager's office. Please follow me, will ya?")
...
class SerialThread(threading.Thread):
    def __init__(self):
        ...
        self.command = None
        try:
            self.ser = serial.Serial('COM6', 115200, timeout=.025, write_timeout=.025)  # Adjust to your Arduino's serial port
...
def decode_qr_code(frame, last_detected_time, cooldown, allow_detection):
    ...
    for qr_code in qr_codes:
        ...
        if qr_data == "Station":
            ...
            stop_robot = True
            stop_duration = 3  # Pause for 20 seconds
            last_detected_time = current_time
...
class FrameProcessingThread(threading.Thread):
    def __init__(self, video_thread):
        ...
        self.final_stop = False  # Flag to indicate the final stop
        ...
        while self.running:
            ...
            if self.stop_robot:
                ...
                if elapsed_time < self.stop_duration or self.stop_duration == float('inf'):
                    ...
                    while time.time() - start_turn_time < 2:
                        send_wheel_command(-3, 3)
                    ...
                    self.final_stop = True  # Set the final stop flag

            if self.final_stop:
                print("Final stop. Closing the program.")
                send_wheel_command(0, 0)
                self.running = False
                break
